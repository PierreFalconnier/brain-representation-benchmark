...
from representation_benchmark_project.utils import pylogger
...
def register_omegaconf_resolvers() -> None:
    ...
    OmegaConf.register_new_resolver(
        "assert",
        lambda condition, throw_on_fail=True: _assert(
            condition, throw_on_fail=throw_on_fail, replace=True
        ),
    )
    OmegaConf.register_new_resolver(
        "op", lambda op, *args: getattr(operator, op)(*args), replace=True
    )
    OmegaConf.register_new_resolver(
        "op.ternary",
    ...
    OmegaConf.register_new_resolver(
        "call", lambda fn_path, *args: import_from_module(fn_path)(*args), replace=True
    )
